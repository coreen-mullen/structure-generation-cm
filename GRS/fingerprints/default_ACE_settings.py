...
def get_default_settings(elems,nshell=1.0,return_range=True,apply_shift=False,metal_max=True,inner_fraction=0.25):
    ...
    fac_per_elem = {'H':0.61,'Be':0.52,'W':0.45,'Cr':0.5,'Fe':0.5,'Si':0.5,'V':0.5}
    for bond in bonds:
        rcmini,rcmaxi = default_rc(bond)
        # try to use tabulated factors for rescaling bond lengths
        try:
            defaultri = (rcmaxi+rcmini)/np.sum([fac_per_elem[elem]*2 for elem in bond])
        # if not tabulated, return to default
        except KeyError:
            defaultri = (rcmaxi+rcmini)/np.sum([1.0 for elem in bond])
